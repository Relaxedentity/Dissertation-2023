...
int DetermineCubeFace(vec3 p) {
    ...
    vec3 direction = p - (cubeModelMatrix*vec4(vec3(0.0),1.0)).xyz;
    float maxDot = -1.0;
    int faceIndex = -1;
...
vec2 getCubeUVFromPoint(vec3 worldPosition, mat4 modelMatrix, vec2 textureDimensions) {
    int temp = DetermineCubeFace(worldPosition);
...
float CubeSDF(vec3 p, vec3 b) {
    vec3 q = abs((inverse(cubeModelMatrix) * vec4(p, 1.0)).xyz) - b;
...
float SceneSDF(vec3 p) {
    float cube = CubeSDF(p, vec3(0.5))*200;
...
void main()
{
    ...
    for (int i = 0; i < maxSteps; i++){
        ...
        {
            if(objectType == 2){
                vec2 uv = getCubeUVFromPoint(p, cubeModelMatrix ,texSize);
