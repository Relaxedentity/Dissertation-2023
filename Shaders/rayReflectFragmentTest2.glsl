...
float SceneSDF(vec3 p) {
    ...
    if(cube <heightmap && cube < cube2 && cube < cube3){
        objectType = 2;
    	temp = cubeModelMatrix;
        return cube;
    }
    if(heightmap <cube && heightmap < cube2 && heightmap < cube3){
        objectType = 3;
        return heightmap;
    }
    if(cube2 <heightmap && cube2 < cube && cube2 < cube3){
	objectType = 2;
	temp = cubeModelMatrix2;
        return cube2;
    }
    if(cube3 <heightmap && cube3 < cube && cube3 < cube2){
	objectType = 2;
